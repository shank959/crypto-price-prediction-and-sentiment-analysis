...
class OlsModel:
    ...
    def train(self, df_target, df_factor1, df_factor2, df_factor3):
        """
        Train a multiple linear regression model using three factors to predict returns.

        Args:
            df_target (pd.DataFrame): Target returns, indexed by datetime and symbol.
            df_factor1 (pd.DataFrame): First factor (e.g., volatility), indexed by datetime and symbol.
            df_factor2 (pd.DataFrame): Second factor (e.g., momentum), indexed by datetime and symbol.
            df_factor3 (pd.DataFrame): Third factor (e.g., volume), indexed by datetime and symbol.

        Saves:
            - 'submit.csv': Submission file with predicted returns.
            - 'check.csv': File with true returns for validation.
        Prints:
            - Model coefficients and intercept.
            - Weighted Spearman correlation coefficient.
        """
        # Convert wide DataFrames to long format Series (multi-index: datetime, symbol)
        ...
        # Predict returns and add to DataFrame
        ...
        # Match submission IDs and fill missing predictions with zero
        df_submission_id = pd.read_csv(
            "/kaggle/avenir-hku-web/submission_id.csv"
        )
        id_list = df_submission_id["id"].tolist()
        df_submit_competition = df_submit[df_submit['id'].isin(id_list)]
        missing_elements = list(
            set(id_list) - set(df_submit_competition['id'])
        )
        ...
        print(df_submit_competition.shape)
        df_submit_competition.to_csv("submit.csv", index=False)

        # Prepare check DataFrame with true returns
        ...
        # Evaluate model performance using weighted Spearman correlation
